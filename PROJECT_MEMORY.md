...
- broke up the old `apps-script/src/preferences.gs` monolith by extracting Phase 11 AI recommendation import/apply logic into `apps-script/src/ai-recommendations.gs`, then splitting the control-surface subsystem across `apps-script/src/control-surface-setup.gs`, `apps-script/src/control-surface-validation.gs`, and `apps-script/src/control-surface-status.gs`
